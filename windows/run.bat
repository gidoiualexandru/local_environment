...
echo Checking if WSL 2 is installed and enabled...

echo Checking if WSL executable is available...

where wsl.exe >nul 2>&1
if errorlevel 1 (
    echo WSL is not installed or enabled. Starting installation...
    ...
    echo Downloading and installing WSL2 Linux kernel update...
	set "KERNEL_MSI=%TEMP%\wsl_update.msi"
    if not exist "!KERNEL_MSI!" (
        powershell -NoProfile -ExecutionPolicy Bypass -Command "Invoke-WebRequest -Uri 'https://wslstorestorage.blob.core.windows.net/wslblob/wsl_update_x64.msi' -OutFile '!KERNEL_MSI!' -UseBasicParsing"
    )
    ...
    echo WSL installation/enabling is complete but requires a restart.
...
rem At this point, WSL executable exists
echo WSL executable found. Verifying WSL version of installed distros...

rem Initialize flag
set "HAS_WSL2=0"

rem Parse wsl --list --verbose output
for /f "skip=1 tokens=1,2,3,4 delims= " %%A in ('wsl --list --verbose 2^>nul') do (
    rem %%A=index, %%B=distro name, %%C=state, %%D=version number
    set "VER=%%D"
    rem Validate VER contains only digits before comparing
    setlocal enabledelayedexpansion
    set "VER_EXP=!VER!"
    endlocal & set "VER=!VER_EXP!"

    for /f "delims=0123456789" %%N in ("!VER!") do set "VER="
...
:WSL2Found
if "%HAS_WSL2%"=="1" (
    echo Found at least one WSL 2 distro installed and/or running.
) else (
    echo No WSL 2 distro found.
    echo Setting WSL default version to 2...
    wsl --set-default-version 2 >nul 2>&1
    echo Default WSL version set to 2.
)

rem Check if any WSL distro is installed
...
if errorlevel 1 (
    ...
    echo Ubuntu has been installed. A restart is required to complete the setup.
    ...
    if errorlevel 2 (
        ...
        echo Restarting in 10 seconds...
        shutdown /r /t 10 /c "Restarting to complete WSL distro installation"
        exit /b 0
    )
) else (
...
if errorlevel 1 (
    ...
    if errorlevel 0 (
        ...
        if errorlevel 1 (
            echo winget failed to install Podman.
...
podman run --name %CONTAINER_NAME% --network mynetwork --ip 172.168.0.10 --restart=always -d -p %SOLR_PORT%:%SOLR_PORT% solr:latest
...
%POWERSHELL% -Command "$json = '{\"add-field\": [{\"name\": \"job_link\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
...
%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"job_title\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"company\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"company_str\", \"type\": \"string\", \"stored\": true, \"indexed\": true, \"multiValued\": false, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"hiringOrganization.name\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"country\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"city\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"county\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

rem Copy-field rules
%POWERSHELL% -Command ^
"$json = '{\"add-copy-field\": { \"source\": \"job_link\", \"dest\": \"_text_\" }}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-copy-field\": { \"source\": \"job_title\", \"dest\": \"_text_\" }}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-copy-field\": { \"source\": \"company\", \"dest\": [\"_text_\", \"company_str\", \"hiringOrganization.name\"] }}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-copy-field\": { \"source\": \"hiringOrganization.name\", \"dest\": \"hiringOrganization.name_str\" }}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-copy-field\": { \"source\": \"country\", \"dest\": \"_text_\" }}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

%POWERSHELL% -Command ^
"$json = '{\"add-copy-field\": { \"source\": \"city\", \"dest\": \"_text_\" }}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/schema"

rem Add url field to logo core
%POWERSHELL% -Command ^
"$json = '{\"add-field\": [{\"name\": \"url\", \"type\": \"text_general\", \"stored\": true, \"indexed\": true, \"multiValued\": true, \"uninvertible\": true}]}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
...
%POWERSHELL% -Command "$json = '{\"authentication\": {\"blockUnknown\": true, \"class\": \"solr.BasicAuthPlugin\", \"credentials\": { \"solr\": \"IV0EHq1OnNrj6gvRCwvFwTrZ1+z1oBbnQdiVC3otuq0= Ndd7LKvVBAaZIF0QAVi1ekCfAJXr1GGfLtRUXhgrF8c=\" }, \"realm\": \"My Solr users\", \"forwardCredentials\": false }, \"authorization\": { \"class\": \"solr.RuleBasedAuthorizationPlugin\", \"permissions\": [ { \"name\": \"security-edit\", \"role\": \"admin\" } ], \"user-role\": { \"solr\": \"admin\" } } }'; $json | Out-File -FilePath '%SEC_JSON%' -Encoding UTF8"
podman cp "%SEC_JSON%" %CONTAINER_NAME%:/var/solr/data/security.json
podman restart %CONTAINER_NAME%

rem Add suggest component and handler to jobs core
%POWERSHELL% -Command "$json = '{\"add-searchcomponent\": {\"name\": \"suggest\", \"class\": \"solr.SuggestComponent\", \"suggester\": {\"name\": \"jobTitleSuggester\", \"lookupImpl\": \"FuzzyLookupFactory\", \"dictionaryImpl\": \"DocumentDictionaryFactory\", \"field\": \"job_title\", \"suggestAnalyzerFieldType\": \"text_general\", \"buildOnCommit\": true, \"buildOnStartup\": false}}}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
...
%POWERSHELL% -Command "$json = '{\"add-requesthandler\": {\"name\": \"/suggest\", \"class\": \"solr.SearchHandler\", \"startup\": \"lazy\", \"defaults\": {\"suggest\": true, \"suggest.dictionary\": \"jobTitleSuggester\", \"suggest.count\": 10}, \"components\": [\"suggest\"]}}'; $json | Out-File -FilePath '%TMP_JSON%' -Encoding UTF8"
podman cp "%TMP_JSON%" %CONTAINER_NAME%:/tmp/post.json
podman exec %CONTAINER_NAME% curl -s -X POST -H "Content-Type: application/json" --data-binary @/tmp/post.json "http://localhost:%SOLR_PORT%/solr/%CORE_NAME_2%/config"
